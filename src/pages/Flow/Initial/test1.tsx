...
const DetailContent = (props: any) => {
  ...
  const handleAdd = (childList: any) => {
    ...
    detail &&
      (data = {
        ...detail,
        formChildlist: [...detail?.formChildlist, newList],
      });
    setDetail(data as any);
  };

  const handleRemove = (childList: any) => {
    const newList: any = detail?.formChildlist.filter(
      item => item.sort !== childList.sort,
    );
    const addList = newList.filter(
      (item: any) => item.canAdd === true && item.id === childList.id,
    );
    addList.length === 0 &&
                ...
                <div key={item.sort}>
                  <Card>
                          ...
                          return (
                            <div key={formList.id + '-' + item.sort}>
                              {getFieldDecorator(`${formId}`, {
                                rules: [
                                  { required: formList.isRequired === 1 },
                                ],
                                initialValue:
                                  formList.defaultShowValue ||
                    ...
                    {item?.type === 1 && item?.canAdd && (
                      <Card.Footer
                        content={
                          <a onClick={_ => handleAdd(item)}>添加{item?.name}</a>
                        }
                      />
                    )}
                  </Card>
