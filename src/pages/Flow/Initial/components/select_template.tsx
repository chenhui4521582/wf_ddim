...
export default (props: ISelectParams) => {
  ...
  if (type === 'select') {
    ...
    const { list } = useVacationType();
    dataSource = loopVacationType(list);
    cols = 1;
  } else if (type === 'addSignType') {
    const { signType } = useAddSignType();
    dataSource = loopSignType(signType);
    cols = 1;
  }
  ...
  return (
    <Picker
      ...
      data={dataSource}
      cols={cols}
      extra={type === 'user' ? newProps.username : null}
    >
      <List.Item arrow="horizontal" wrap={true}>
        <span className={newProps.disabled ? styles.selectDisabled : null}>
          {name}
        </span>
...
const loopVacationType = (data: any[]) => {
  return data?.map((item: any) => {
    const result: IPickerData = {
      value: item.typeId + '---' + item.name,
      label: item.name,
    };
    return result;
  });
